...
const debug = createDebugger('dpack:esbuild')
...
export const buildEsbuildPlugin = (config: ResolvedConfig): Plugin => {
  return {
    name: 'dpack:esbuild-transpile',
    async configResolved(config) {
      await initTSConfck(config)
    },
    async renderChunk(code, chunk, opts) {
      // @ts-expect-error
      if (opts.__dpack_skip_esbuild__) {
        return null
      }
...
function reloadOnTsconfigChange(changedFile: string) {
  if (
    ...
    // 重置tsconfck，以便重新编译时能使用up2date配置。
